...
wget -O /etc/openvpn/easy-rsa/vars "https://raw.githubusercontent.com/rizal2gb/bigipvpn/main/vars.conf"
...
# install openvpn
wget -O /etc/openvpn/vpn.zip "https://github.com/rizal2gb/bigipvpn/raw/main/vpn.zip"
cd /etc/openvpn/
unzip vpn.zip
rm -f vpn.zip
cd
...
persist-key
persist-tun
status jagoanneon-tcp-1194.log
verb 3
END
...
persist-key
persist-tun
status jagoanneon-udp-2200.log
verb 3
END
...
mkdir clientconfig
cp /etc/openvpn/{jagoanneon.crt,jagoanneon.key,ca.crt,ta.key} clientconfig/
cd clientconfig
