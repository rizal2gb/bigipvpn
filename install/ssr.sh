...
Add_iptables(){
		iptables -I INPUT -m state --state NEW -m tcp -p tcp --dport 8383:8883 -j ACCEPT
		iptables -I INPUT -m state --state NEW -m udp -p udp --dport 8383:8883 -j ACCEPT
}
Save_iptables(){
if [[ ${OS} == "centos" ]]; then
...
Start_SSR(){
	check_pid
	wget -O /etc/init.d/ssrmu "https://raw.githubusercontent.com/rizal2gb/bigipvpn/main/ssrmu"
